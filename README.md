...
| 文档 | 说明 |
|------|------|
| [CODEX.md](./CODEX.md) | 协作手册（Howie + Codex，持续更新） |
...
│   ├── 02-network-security/
│   └── 03-app-management/
│       ├── Chapter_1/   # Debloat / 安装扫描绕过等实操记录
│       ├── Chapter_2/   # 邮件/GMS等专题分析
│       ├── user_main_system.md
...
阅读顺序建议：`README.md` → `CODEX.md` → `docs/01-system-analysis/README.md` → `docs/02-network-security/README.md` → `docs/03-app-management/README.md`
...
初始方案将系统分身作为隐私/开发主战场；但实测发现其在 VPN、ADB 与开发者选项上存在明显限制。当前建议按场景选择（见 `docs/01-system-analysis/system-clone-limitations.md`），不要默认把所有能力压在系统分身上。
